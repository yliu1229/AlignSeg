...
def display_allsegments(image, masks, fname="test", figsize=(5, 5), alpha=0.6):
    N = 5   # num of colors
    # Generate random colors
...
if __name__ == '__main__':
    ...
    gc_token_cls = gc_token_cls.reshape(1, w_spatial_res, w_spatial_res, -1).permute(0, 3, 1, 2)  # (B, num_query, 30, 30)

    # get segmentation class
    cls_masks = (gc_token_cls == gc_token_cls.max(dim=1, keepdim=True)[0]).float()
    cls_masks = cls_masks.squeeze(0)

    masks = []
    for i in range(cls_masks.shape[0]):
        mask = cls_masks[i].unsqueeze(0).unsqueeze(0)
        mask = nn.functional.interpolate(mask, scale_factor=args.patch_size, mode="nearest")[0].cpu().numpy()
        masks.append(mask[0])
    display_allsegments(image, masks, fname=args.output_dir)
    # display_segments(image, masks, fname=args.output_dir)
