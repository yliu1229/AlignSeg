...
class VisionTransformer(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, inputs, nmb_crops=(1,0), last_self_attention=False):
        ...
        for end_idx in idx_crops:
            ...
            if last_self_attention:
                _out, _attn = _out
            spatial_tokens = _out[:, 1:]  # remove CLS token
            spatial_tokens = spatial_tokens.reshape(-1, self.embed_dim)  # [B*196/36, embed_dim]
...
if __name__ == '__main__':
    clsQueries = nn.Embedding(2, 6)
    print(clsQueries.weight)
    trunc_normal_(clsQueries.weight, std=.02)
    print(clsQueries.weight, clsQueries.weight.mean(), clsQueries.weight.sum())

    cls = nn.Parameter(torch.zeros(1, 1, 5))
    trunc_normal_(cls, std=.02)
    print(cls, cls.mean(), cls.sum())
